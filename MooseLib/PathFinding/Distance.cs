...
public struct Distance : IComparable<Distance>, IEquatable<Distance>
{
    ...
    private Distance(float meters) => Meters = meters;

    public float Meters { get; }

    public static Distance FromMeters(float meters) => new Distance(meters);

    public static Distance BeweenPositions(Vector2 a, Vector2 b)
    {
        var sX = a.X;
        var sY = a.Y;
        var eX = b.X;
    ...
    public override int GetHashCode() => -1609761766 + Meters.GetHashCode();
}
